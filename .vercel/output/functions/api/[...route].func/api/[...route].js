...
async function registerCredentials(req, res) {
    if (req.method !== 'POST')
        return sendError(res, 405, 'Method not allowed');
    try {
...
async function verifyGoogle(req, res) {
    ...
    try {
        ...
        if (!idToken)
            return sendError(res, 400, 'Missing idToken');
...
async function verifyTokenEndpoint(req, res) {
    ...
    try {
        ...
        const payload = verifyToken(token);
        if (!payload)
            return sendJson(res, 401, { __kind__: 'err', err: 'Invalid token' });
        const user = await authService.getUser(payload.userId);
        if (!user)
            return sendJson(res, 404, { __kind__: 'err', err: 'User not found' });
...
async function deleteAccount(req, res) {
    ...
    try {
        ...
        if (!token)
            return sendJson(res, 401, { __kind__: 'err', err: 'Missing authentication token' });
        const payload = verifyToken(token);
        if (!payload)
            return sendJson(res, 401, { __kind__: 'err', err: 'Invalid token' });
...
async function getSalt(req, res, username) {
    if (req.method !== 'GET')
        return sendError(res, 405, 'Method not allowed');
    try {
        if (!username)
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (!key || typeof key !== 'string')
                return sendError(res, 400, 'Missing key');
            if (!ALLOWED_KEYS.has(key))
                return sendError(res, 400, `Unknown setting key: ${key}`);
            if (value === undefined || value === null)
                return sendError(res, 400, 'Missing value');
        ...
        if (req.method === 'PUT') {
            ...
            if (!settings || typeof settings !== 'object')
                return sendError(res, 400, 'Missing settings object');
            const updates = [];
...
async function adminAds(req, res) {
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (!provider || !VALID_AD_PROVIDERS.has(provider))
                return sendError(res, 400, 'Invalid or missing provider');
            const update = { updated_at: new Date().toISOString() };
            for (const [key, value] of Object.entries(fields)) {
                if (VALID_AD_FIELDS.has(key))
                    update[key] = value;
            }
        ...
        if (req.method === 'PUT') {
            ...
            if (!Array.isArray(configs))
                return sendError(res, 400, 'Expected configs array');
            const results = [];
            for (const cfg of configs) {
                if (!cfg.provider || !VALID_AD_PROVIDERS.has(cfg.provider))
                    continue;
                const update = { updated_at: new Date().toISOString() };
                for (const [key, value] of Object.entries(cfg)) {
                    if (key !== 'provider' && VALID_AD_FIELDS.has(key))
                        update[key] = value;
                }
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (!id || !VALID_TIERS.has(id))
                return sendError(res, 400, 'Invalid or missing plan id');
            const update = {
                id,
                name: name || id,
        ...
        if (req.method === 'PUT') {
            ...
            if (!Array.isArray(plans))
                return sendError(res, 400, 'Expected plans array');
            const results = [];
            for (const plan of plans) {
                if (!plan.id || !VALID_TIERS.has(plan.id))
                    continue;
...
function resolvePath(req) {
    ...
    const catchAllParam = req.query['...route'] ?? req.query.route;
