...
export class AuthService {
     ...
     */
    async registerWithCredentials(username, email, passwordHash, salt) {
        try {
     ...
     */
    async loginWithCredentials(username, passwordHash) {
        try {
            ...
            return { __kind__: 'ok', ok: credential.user_id };
        }
        catch (error) {
    ...
    async verifyGoogleOAuth(idToken) {
        try {
            ...
            if (existingLink) {
                ...
                return { __kind__: 'ok', ok: existingLink.user_id };
            }
     ...
     */
    async resolveUniqueUsername(base) {
        let candidate = base;
        let n = 1;
        while (true) {
